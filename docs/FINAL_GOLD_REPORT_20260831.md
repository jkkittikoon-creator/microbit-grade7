...
> **SUPERSEDED 2 Sep 2026:** This 31 Aug checkpoint records the earlier PARTIAL state only. The release later completed TEST v10 GOLD and the Production deployment pointer was moved to v40. ~~It is now FINAL PRODUCTION GOLD — PASS / CLOSED.~~ **Corrected later the same day: version 40 turned out to be version 39 relabelled and carries none of the TEST v10 work, so the release is NOT closed.** Current authority: `docs/PRODUCTION_V40_RELEASE_CHECKPOINT_20260902.md` (read the correction at the top first), then `docs/FINAL_PRODUCTION_GOLD_REPORT_20260902.md` and `DEV_CONTINUATION.md`.
