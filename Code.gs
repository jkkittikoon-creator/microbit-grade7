...
const OPTIONAL_EVIDENCE_RULES = Object.freeze({
  ...
  codingLab: Object.freeze({
    ...
    readingMin: -2048,
    readingMax: 2048,
    readingKeys: Object.freeze(['flat', 'leftSmall', 'leftMax', 'rightMax', 'shake']),
    thinkingChoices: Object.freeze({
      q1: Object.freeze(['handRange', 'sensorMax', 'ledRange']),
      q2: Object.freeze(['map', 'rawPlot', 'clipRaw'])
    }),
    thinkingAnswers: Object.freeze({ q1: 'handRange', q2: 'map' }),
    systemTestChoices: Object.freeze(['edge', 'missing', 'blink']),
    correctSystemTestAnswer: 'missing',
    maxThinkingAttempts: 20,
...
function normalizeSensorReading_(value, rules) {
  const text = value === null || value === undefined ? '' : String(value).trim();
  if (!/^-?\d{1,4}$/.test(text)) return '';
  const number = Number(text);
  if (!Number.isFinite(number) || Math.floor(number) !== number) return '';
  if (number < rules.readingMin || number > rules.readingMax) return '';
  return String(number);
}

function normalizeSensorReadings_(value, rules) {
  const source = isPlainRecord_(value) ? value : {};
  const clean = {};
  rules.readingKeys.forEach(function (key) {
    const reading = normalizeSensorReading_(source[key], rules);
    if (reading !== '') clean[key] = reading;
  });
  return clean;
}

function normalizeThinkingAttempt_(incomingValue, trustedValue, choices, answer, rules) {
  const incoming = isPlainRecord_(incomingValue) ? incomingValue : {};
  const trusted = isPlainRecord_(trustedValue) ? trustedValue : {};
  const incomingAnswer = normalizeEvidenceChoice_(incoming.answer, choices);
  const trustedAnswer = normalizeEvidenceChoice_(trusted.answer, choices);
  const selectedAnswer = incomingAnswer || trustedAnswer;
  const incomingAttempts = Math.max(
    0,
    Math.min(rules.maxThinkingAttempts, Math.floor(Number(incoming.attempts) || 0))
  );
  const trustedAttempts = Math.max(
    0,
    Math.min(rules.maxThinkingAttempts, Math.floor(Number(trusted.attempts) || 0))
  );
  ...
  const clean = {
    attempts: attempts,
    hintViewed: attempts > 0 &&
      (trusted.hintViewed === true || incoming.hintViewed === true),
    correct: selectedAnswer === answer
  };
  if (selectedAnswer) clean.answer = selectedAnswer;
  return clean;
}

function normalizeSensorThinking_(incomingValue, trustedValue, rules) {
  const incoming = isPlainRecord_(incomingValue) ? incomingValue : {};
  const trusted = isPlainRecord_(trustedValue) ? trustedValue : {};
  const clean = {};
  Object.keys(rules.thinkingAnswers).forEach(function (key) {
    clean[key] = normalizeThinkingAttempt_(
      incoming[key],
      trusted[key],
      rules.thinkingChoices[key],
      rules.thinkingAnswers[key],
      rules
    );
  });
  return clean;
...
function normalizeCodingLabEvidence_(incomingValue, trustedValue, unlocked) {
  ...
  const readingSource = isPlainRecord_(incoming.sensorReadings)
    ? incoming.sensorReadings
    : trusted.sensorReadings;
  const sensorReadings = normalizeSensorReadings_(readingSource, rules);
  const readingsComplete = rules.readingKeys.every(function (key) {
    return hasOwn_(sensorReadings, key);
  });
  if (Object.keys(sensorReadings).length) clean.sensorReadings = sensorReadings;

  const sensorThinking = normalizeSensorThinking_(
    incoming.sensorThinking,
    trusted.sensorThinking,
    rules
  );
  const thinkingComplete = Object.keys(rules.thinkingAnswers).every(function (key) {
    return sensorThinking[key] && sensorThinking[key].correct === true;
  });
  clean.sensorThinking = sensorThinking;

  clean.systemTest = normalizeThinkingAttempt_(
    incoming.systemTest,
    trusted.systemTest,
    rules.systemTestChoices,
    rules.correctSystemTestAnswer,
    rules
  );
