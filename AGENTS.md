...
node tests/pretest-path.test.mjs Code.gs
```

They cover, in order: Free Navigation Preview reward gating; the editor-only guard on the installer and maintenance entry points; per-objective Mastery computation; and the Pre-test plus its adaptive path, including the guard that the Pre-test never shifts `completedSections`. None is evidence for anything outside its own scope, and none covers the frontend.

The frontend can be exercised offline by serving `index.html` from a local Node server that injects a `google.script.run` stub before the page's own scripts run, with canned responses generated by loading the real `Code.gs` into a `vm` context. Installing the stub after load does not work: `getPublicConfig` has already failed by then, `lessonSections` stays empty, and `refreshLessonNavigation` returns early.
